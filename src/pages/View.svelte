    ...
    if (!isTauri) {
        ...
        if (passwordId == null) {
            ...
            invoke("get_password", {
                ...
                password_res.username = password_res.username.replace(
                    /^"(.*)"$/,
                    "$1",
                );
                password_res.decrypted_password =
                    password_res.decrypted_password
                        ? password_res.decrypted_password.replace(
                              /^"(.*)"$/,
                              "$1",
                          )
                        : "";
                password_res.url = password_res.url.replace(/^"(.*)"$/, "$1");
                password_res.notes = password_res.notes.replace(
                    /^"(.*)"$/,
                    "$1",
                );
                // replace empty strings with n/a
                password_res.name =
                    password_res.name === "" ? "n/a" : password_res.name;
                password_res.username =
                    password_res.username === ""
                        ? "n/a"
                        : password_res.username;
                password_res.decrypted_password =
                    password_res.decrypted_password === ""
                        ...
                        : password_res.decrypted_password;
                password_res.url =
                    password_res.url === "" ? "n/a" : password_res.url;
                password_res.notes =
                    password_res.notes === "" ? "n/a" : password_res.notes;
...
<main class="container">
    ...
    <div class="row">
        ...
        <input
            ...
            class="form-control"
            id="password"
            placeholder={$_("viewpw.password")}
            value={password.decrypted_password}
            readonly
        />
    </div>
    <div class="row">
    ...
    <div class="row">
        ...
        <input
            ...
            class="form-control"
            id="notes"
            placeholder={$_("viewpw.notes")}
            value={password.notes}
            readonly
        />
    </div>
    <button type="button" class="btn btn-primary" on:click={() => closeWindow()}
