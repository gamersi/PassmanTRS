<script lang="ts">
    ...
    function checkMasterPassword() {
        ...
        if (storedPassword) {
            const storedTime =
                localStorage.getItem("masterPassword_time") || "0";
            const currentTime = new Date().getTime();
            if (currentTime - parseInt(storedTime) <= EXPIRATION_TIME) {
                invoke("validate_master_password", {
        ...
        <div class="contents">
            ...
            <form
                on:submit|preventDefault={(e) => {
                    ...
                    invoke("validate_master_password", { password }).then(
                        (res) => {
                            ...
                            if (res) {
                                ...
                                e.target[0].placeholder = $_(
                                    "masterpassword.enter",
                                );
